...
#' Fitted value and realized value
y <- (df$Rm - df$Rf)[2:581]
...
#' Define function to calculate rolling coefficients
...
#' Define a function to construct predicted value
...
#' Calculate out-of-sample statistics from Welch and Goyal, 2008
rsq_os.b.1 <- 1 - sum((rm_os - rmhat1)^2)/sum((rm_os - rmbar)^2)
rsq_os.b.2 <- 1 - sum((rm_os - rmhat2)^2)/sum((rm_os - rmbar)^2)

#' Calculate in-sample R-squared for the out-of-sample period
...
#' Define a function to modify predicted value following Campbell and Thompson, 2008
...
#' e.
#' repeat the calculation of part b
y <- (df$Rm - df$Rf)[3:581]
y_ls <- subsample(y)
...
#' Repeat the calculation of part c
xbar <- x[(579 - 360 + 2):579]
...
#' f.
#' Calculate out-of-sample R-squared when the theoretical restrictions are fully imposed
rmhat <- x - 1
